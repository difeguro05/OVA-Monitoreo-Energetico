    ...
    function handleMessage(msg) {
        switch (msg.type) {
            case 'rms':
                ...
                SessionBuffer.push(msg.voltage, msg.current, msg.power);
                break;
            case 'wave':
            ...
            case 'ack':
                ...
                if (msg.cmd === 'START') {
                    document.getElementById('sessionInfo').textContent = 'Sesión activa';
                    document.getElementById('btnStart').disabled = true;
                    SessionBuffer.start();
                } else if (msg.cmd === 'STOP') {
                    document.getElementById('sessionInfo').textContent = 'Sin sesión activa';
    ...
    const FileManager = (() => {
        ...
        function renderFileList(files) {
            ...
            container.innerHTML = files.map(f => `
                <div class="file-item">
                    <div class="file-info" onclick="FileManager.plotFile('${f.name}')" style="cursor:pointer;flex:1">
                        <span class="file-name">${f.name}</span>
                        <span class="file-size">${(f.size / 1024).toFixed(1)} KB</span>
                    </div>
                    <button class="file-btn download-btn" onclick="FileManager.downloadFile('${f.name}')" title="Descargar CSV">↓</button>
                </div>
            `).join('');
        }

        function plotFile(name) {
            pendingDownload = false;
            SerialManager.send(`READ ${name}`);
            logSerial('INFO', `Cargando ${name}...`);
        }
        ...
        function finishReceiving() {
            ...
            if (pendingDownload) {
                ...
                logSerial('INFO', `Descargado: ${receivingName}`);
            }
        ...
        return { listFiles, renderFileList, plotFile, downloadFile, startReceiving, receiveFileData, finishReceiving };
    })();
    ...
    const HistoryChart = (() => {
        const COLORS = {
            voltage: { line: '#3b82f6', fill: 'rgba(59,130,246,0.08)' },
            current: { line: '#22c55e', fill: 'rgba(34,197,94,0.08)'  },
            power:   { line: '#f59e0b', fill: 'rgba(245,158,11,0.08)' },
        };

        const chartOpts = (color, fill, unit) => ({
            type: 'line',
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        titleColor: '#4a5568', bodyColor: '#dce3ed',
                        callbacks: { label: ctx => `${ctx.parsed.y.toFixed(3)} ${unit}` }
                    }
                },
                scales: {
                    x: {
                        ticks: { color: '#2a3340', font: { size: 9 }, maxRotation: 0, autoSkipPadding: 40 },
                        grid: { color: '#111820' }
                    },
                    y: {
                        ticks: { color: '#2a3340', font: { size: 9 } },
                        grid: { color: '#111820' }
                    }
                }
            }
        ...
        function init() {
            charts.voltage = new Chart(document.getElementById('chartVoltage'), chartOpts(COLORS.voltage.line, COLORS.voltage.fill, 'V'));
            charts.current = new Chart(document.getElementById('chartCurrent'), chartOpts(COLORS.current.line, COLORS.current.fill, 'A'));
            charts.power   = new Chart(document.getElementById('chartPower'),   chartOpts(COLORS.power.line,   COLORS.power.fill,   'W'));
        }

        function loadData(timestamps, voltage, current, power) {
            const labels = timestamps.map(t => `${t.toFixed(0)}s`);
            [
                { chart: charts.voltage, data: voltage },
                { chart: charts.current, data: current },
                { chart: charts.power,   data: power   },
            ].forEach(({ chart, data }) => {
                if (!chart) return;
            ...
            // Activar tags EN VIVO
            ['liveTagV','liveTagI','liveTagP'].forEach(id => {
        ...
        return { init, loadData, stopLive, setMetric: () => {}, exportChart: () => {} };
    })();
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        Gauges.init();
        Oscilloscope.init();
        // Buffer de sesión en tiempo real
        const SessionBuffer = (() => {
            let timestamps = [], voltages = [], currents = [], powers = [];
            let sessionStart = null;

            function push(v, i, p) {
                if (!sessionStart) return;
                const t = (Date.now() - sessionStart) / 1000;
                timestamps.push(t);
                voltages.push(v);
                currents.push(i);
                powers.push(p);
            ...
            function stop() { sessionStart = null; HistoryChart.stopLive(); }
